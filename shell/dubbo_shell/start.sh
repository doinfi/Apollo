...
# data
jvm_cmd_param_very_high="java -Xms1024m -Xmx2048m -XX:ErrorFile=${log_dir}/hs_err_pid_%p.log -jar "

# rest/user/open
jvm_cmd_param_high="java -Xms512m -Xmx1024m -XX:ErrorFile=${log_dir}/hs_err_pid_%p.log -jar "

# other
jvm_cmd_param_normal="java -Xms256m -Xmx512m -XX:ErrorFile=${log_dir}/hs_err_pid_%p.log -jar "

jvm_cmd=${jvm_cmd_param_normal}
if [[ "${microServiceName}" == "data" ]]
then
  jvm_cmd=${jvm_cmd_param_very_high}
elif [[ "${microServiceName}" == "rest" || "${microServiceName}" == "user" || "${microServiceName}" == "open" || "${microServiceName}" == "thirdparty" || "${microServiceName}" == "dataParser" ]]
then
