...
// http://es5.github.com/#x15.3.4.5
    ...
    defineProperties(FunctionPrototype, {
        bind: function bind(that) { // .length is 1
            ...
            bound = $Function('binder', 'return function (' + array_join.call(
                boundArgs, ',') + '){ return binder.apply(this, arguments); }')(
                binder);
...
// https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Array/every
    defineProperties(ArrayPrototype, {
        every: function every(callbackfn/*, thisArg*/) {
            ...
            for (var i = 0; i < length; i++) {
                if (i in self && !(typeof T === 'undefined' ? callbackfn(
                        self[i], i, object) : callbackfn.call(T, self[i], i,
                        object))) {
                    return false;
                }
            }
        ...
        defineProperties(ArrayPrototype, {
            join: function join(separator) {
                ...
                return originalJoin.call(
                    isString(this) ? strSplit(this, '') : this, sep);
            }
        }, hasStringJoinBug);
    }
...
// http://whattheheadsaid.com/2010/10/a-safer-object-keys-compatibility-implementation
    ...
    var hasAutomationEqualityBug = (function () {
        ...
        for (var k in window) {
            try {
                if (!blacklistedKeys['$' + k] && owns(window, k) && window[k]
...
// can be replaced with require('is-arguments') if we ever use a build process instead
    ...
    var keysHasArgumentsLengthBug = $Object.keys && (function () {
        ...
        return arguments.length !== 1 || argKeys.length !== 1 || argKeys[0]
            !== 1;
    }(1));
...
//
    ...
    var hasNegativeMonthYearBug = new Date(-3509827329600292).getUTCMonth()
        !== 0;
    ...
    if (timeZoneOffset < -720) {
        hasToDateStringFormatBug = aNegativeTestDate.toDateString()
            !== 'Tue Jan 02 -45875';
                ...
                if (this instanceof NativeDate) {
                    ...
                    date = length === 1 && $String(Y) === Y ? // isString(Y)
                        ...
                        length >= 7 ? new NativeDate(Y, M, D, h, m, seconds,
                            millis) : length >= 6 ? new NativeDate(Y, M, D, h,
                            m, seconds) : length >= 5 ? new NativeDate(Y, M, D,
                            h, m) : length >= 4 ? new NativeDate(Y, M, D, h)
                            : length >= 3 ? new NativeDate(Y, M, D) : length
                            >= 2 ? new NativeDate(Y, M) : length >= 1
                                ? new NativeDate(
...
// http://es5.github.com/#x15.7.4.5
    ...
    var toFixedShim = function toFixed(fractionDigits) {
        ...
        if (x > 1e-21) {
            ...
            if (e > 0) {
                ...
                m = toFixedHelpers.numToString() + strSlice(
                    '0.00000000000000000000', 2, 2 + f);
            }
        }
    ...
    defineProperties(NumberPrototype, {
        toPrecision: function toPrecision(precision) {
            return typeof precision === 'undefined' ? originalToPrecision.call(
                this) : originalToPrecision.call(this, precision);
        }
        ...
        StringPrototype.replace = function replace(searchValue, replaceValue) {
            ...
            if (!isFn || !hasCapturingGroups) {
                ...
                var wrappedReplaceValue = function (match) {
                    ...
                    pushCall(args, arguments[length - 2],
                        arguments[length - 1]);
...
// whitespace from: http://es5.github.io/#x15.5.4.20
    ...
    var hasLastIndexBug = StringPrototype.lastIndexOf && 'abcあい'.lastIndexOf(
        'あい', 2) !== -1;
    defineProperties(StringPrototype, {
    ...
    if (!Object.defineProperty || definePropertyFallback) {
        ...
        Object.defineProperty = function defineProperty(object, property,
            ...
            */
                ...
                if (!supportsAccessors && (('get' in descriptor) || ('set'
                        in descriptor))) {
...
// http://es5.github.com/#x15.2.3.7
    if (!Object.defineProperties || definePropertiesFallback) {
        Object.defineProperties = function defineProperties(object,
            properties) {
    ...
    WindowPrototype[removeEventListener] = DocumentPrototype[removeEventListener] = ElementPrototype[removeEventListener] = function (type,
        ...
        for (var index = 0, register; register = registry[index]; ++index) {
            if (register[0] == this && register[1] == type && register[2]
                == listener) {
                return this.detachEvent("on" + type,
    ...
    var cssremunit = function () {
        ...
        matchCSS = function (sheetCSS, link) { // collect all of the rules from the xhr response texts and match them to a pattern
            var clean = removeMediaQueries(sheetCSS).replace(
                /\/\*[\s\S]*?\*\//g, ''), // remove MediaQueries and comments
        ...
        parseCSS = function () { // replace each set of parentheses with evaluated content
            for (var i = 0; i < foundProps.length; i++) {
                css[i] = Math.round(
                    parseFloat(foundProps[i].substr(0, foundProps[i].length - 3)
                        * fontSize)) + 'px';
            }
        ...
        xhr = function (url, callback) { // create new XMLHttpRequest object and run it
            try {
                ...
                var xhr = window.ActiveXObject ? ( new ActiveXObject(
                    'Microsoft.XMLHTTP') || new ActiveXObject(
...
!function (a) {
    ...
    a.matchMedia = a.matchMedia || function (a) {
        ...
        return f.id = "mq-test-1", f.style.cssText = "position:absolute;top:-100em", e.style.background = "none", e.appendChild(
            ...
            return f.innerHTML = '&shy;<style media="' + a
                ...
                d), b = 42 === f.offsetWidth, c.removeChild(e), {
                matches: b,
                media: a
            }
        }
    ...
    if (c.ajax = f, c.queue = d, c.unsupportedmq = g, c.regex = {
        ...
        var h, i, j, k = a.document, l = k.documentElement, m = [], n = [],
            o = [], p = {}, q = 30, r = k.getElementsByTagName("head")[0] || l,
            ...
            t = r.getElementsByTagName("link"), u = function () {
                ...
                for (var s in m) {
                    if (m.hasOwnProperty(s)) {
                        var w = m[s], x = w.minw, y = w.maxw, z = null === x,
                            A = null === y, B = "em";
                        x && (x = parseFloat(x) * (x.indexOf(B) > -1 ? j || u()
                            : 1)), y
                        && (y = parseFloat(y) * (y.indexOf(B) > -1 ? j || u()
                            : 1)), w.hasquery && (z && A || !(z || e >= x) || !(A
                            || y
                            >= e)) || (f[w.media]
                        || (f[w.media] = []), f[w.media].push(
                            n[w.rules]))
                    }
                }
                for (var C in o) {
                    o.hasOwnProperty(C) && o[C] && o[C].parentNode
                    === r && r.removeChild(o[C]);
                }
                o.length = 0;
                for (var D in f) {
                ...
                for (var j = 0; f > j; j++) {
                    ...
                    for (var q = 0; p > q; q++) {
                        l = o[q], g(l) || m.push({
                            ...
                            minw: l.match(c.regex.minw) && parseFloat(RegExp.$1)
                            + (RegExp.$2 || ""),
                            maxw: l.match(c.regex.maxw) && parseFloat(RegExp.$1)
                            + (RegExp.$2 || "")
                        })
                    }
                }
                ...
                for (var b = 0; b < t.length; b++) {
                    ...
                    e && g && !p[e] && (c.styleSheet && c.styleSheet.rawCssText
                        ...
                        : (!/^([a-zA-Z:]*\/\/)/.test(e) && !s || e.replace(
                        RegExp.$1, "").split("/")[0] === a.location.host) && ("//"
